...
export interface SubgroupBar {
  level: string
  distribution: number[] // [%_top, %_second, %_third, %_bottom]
...
export interface QuestionPanelData {
  ...
  trends: TrendBar[]
  historicalAverages: Record<string, Record<string, number>> // year -> level -> avg
}
...
function roundDistribution(dist: number[]): number[] {
  const rounded = dist.map((v) => Math.round(v * 10) / 10)
  const sum = rounded.reduce((a, b) => a + b, 0)
  const diff = Math.round((sum - 100) * 10) / 10
  if (diff !== 0 && rounded.length > 0) {
    // Adjust the largest segment to ensure sum === 100
    let maxIdx = 0
...
export default function QuestionPanel({ data, selectedYear }: QuestionPanelProps) {
  const subgroupData = toStackedData(
    data.subgroups.map((s) => ({ name: s.level, distribution: s.distribution, weightedAverage: s.weightedAverage }))
  )
  const trendData = toStackedData(
    data.trends.map((t) => ({ name: t.year, distribution: t.distribution, weightedAverage: t.weightedAverage }))
  )

  // Build historical averages text per level
  const histYears = Object.keys(data.historicalAverages).sort()
  const otherYears = histYears.filter((y) => y !== selectedYear)

  return (
    <div>
      <h3 className="text-sm font-semibold mb-2">
        Q{data.presentationNumber}: {data.shortTitle}
      </h3>
      ...
      <div className="flex gap-4">
        <StackedBarSection
          title={`${selectedYear} Subgroup Responses`}
          data={subgroupData}
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {data.subgroups.map((sg) => {
            const avgs = otherYears
              .map((y) => {
                const val = data.historicalAverages[y]?.[sg.level]
