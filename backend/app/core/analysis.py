...
def compute_averages(
    ...
    if weight_by_parents and "N Parents Represented" in result_df.columns:
        weights = result_df["N Parents Represented"].cast(pl.Float64).fill_null(1.0)
    else:
        ...
        for response in scale:
            ...
            for level in LEVELS:
                ...
                if weight_by_parents and "N Parents Represented" in df.columns:
                    level_total = level_df["N Parents Represented"].cast(pl.Float64).sum()
                    response_total = (
                        level_df
                        .filter(pl.col(col) == response)
                        ["N Parents Represented"]
                        .cast(pl.Float64)
                        .sum()
                    )
                else:
            ...
            for segment in segments:
                ...
                for level in LEVELS:
                    ...
                    if weight_by_parents and "N Parents Represented" in segment_df.columns:
                        seg_total += level_df["N Parents Represented"].cast(pl.Float64).sum() or 0
                        seg_response += (
                            level_df
                            .filter(pl.col(col) == response)
                            ["N Parents Represented"]
...
def compute_statistics(df: pl.DataFrame, config: SurveyConfig | None = None) -> dict[str, Any]:
    ...
    # Compute averages
    df_with_avgs, weighted_totals = compute_averages(df, weight_by_parents=False, config=config)

    # Compute question totals
    question_totals = calculate_question_totals(df, weight_by_parents=False)
